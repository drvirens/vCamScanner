		...
		const MAUtil::String &id)
{
	...
	ConnectionRequestCommand *connectionRequest = new ConnectionRequestCommand(	mFacebookManager, 0,
...
{
	LikeCommand *likeCmd = new LikeCommand( mFacebookManager, 0 , id);
...
{
	UnlikeCommand *unlikeCmd = new UnlikeCommand( mFacebookManager, 0, id);
...
{
	AddCommand<Comment> *createCommentCmd = new AddCommand<Comment>( mFacebookManager, 0, id);
...
{
	PostOnWallCommand<Link> *postLinkCommand = new PostOnWallCommand<Link>(mFacebookManager, 0, id);
...
{
	PostOnWallCommand<Post> *addPostCommand = new PostOnWallCommand<Post>(mFacebookManager,0,  id);
		...
		const MAUtil::String &id)
{
	PostOnWallCommand<StatusMessage> *postStatusMessageCmd =
			new PostOnWallCommand<StatusMessage>(mFacebookManager, 0, id);
		...
		EVENT_RESPONSE_TYPE response)
{
	AddCommand<EventResponse> *addEventResponseCmd =
			new AddCommand<EventResponse>(mFacebookManager,0,  eventId);
...
void ConnectionRequestCommand::removeUser(FacebookDemoApplication::OpenMenuCommand *openMenuCmd, const MAUtil::String &listId,
		const MAUtil::String &userId )
{
	RemoveCommand<User> *removeUserCommand = new RemoveCommand<User>(mFacebookManager, 0, listId);
...
{
	RemoveCommand<Comment> *removeCommentCmd = new RemoveCommand<Comment>(mFacebookManager, 0, id);
...
{
	RemoveCommand<FriendList> *removeFriendListCmd = new RemoveCommand<FriendList>(mFacebookManager, 0, id);
...
{
	RemoveCommand<Note> *removeNoteCmd = new RemoveCommand<Note>(mFacebookManager, 0, id);
