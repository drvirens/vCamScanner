...
//
//
//	VIREN_LOG("\t\tadding file to the request: fileSize=%d, fileRawData=%d", fileSize, fileRawData);
...
void HttpQueryDownloader::write(const byte* data, int length) {
	...
	VIREN_LOG("HttpQueryDownloader::write, length =%d", length);
...
void HttpQueryDownloader::error(Downloader* downloader, int code)
{
	...
	if (code == 301 || code == 302)
	{
		...
		VIREN_LOG("\t\tRedirect. New URL = %s", url.c_str());
...
void HttpQueryDownloader::connRecvFinished(MAUtil::Connection *conn, int result) {
	VIREN_LOG("HttpQueryDownloader::connRecvFinished %d", result);
...
void HttpQueryDownloader::connReadFinished(MAUtil::Connection *conn, int result) {
	...
	VIREN_LOG("data: %s", mHttpConnectionBuffer);
}

void HttpQueryDownloader::connWriteFinished(MAUtil::Connection *conn, int result) {
	VIREN_LOG("\t\tHttpQueryDownloader::connWriteFinished %d", result);
	...
	if(result<0)
	{
		VIREN_LOG("Error HttpConnection", "write operation failed");
		return;
	}
