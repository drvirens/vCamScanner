...
void Alarm::setSnooze(const SnoozeMode m)
{
    ...
    System::getDelayedActions().setActionTime(
                DelayedActionsBase::WakeUp,
                to_call);

    System::getDelayedActions().setActionTime(
                DelayedActionsBase::DisplayLabel,
                to_call);
...
void Alarm::cancelAllDelayedActions()
{
    ...
    System::getDelayedActions().cancelAction(DelayedActionsBase::WakeUp);
...
void Alarm::onWakeUpAction()
{
    ...
    if(_mode == WakeUpAndGoOut)
    {
        ...
        if(seconds_to_out >= 60 * 1)
        {
            ...
            //Set last alarm
            System::getDelayedActions().setActionTime(DelayedActionsBase::GoOutThird,
                                                      _go_out - Time(0,1));
        }
        if(seconds_to_out >= 60 * 5)
        {
            System::getDelayedActions().setActionTime(DelayedActionsBase::GoOutSecond,
                                                      _go_out - Time(0,5));
        }
        if(seconds_to_out >= 60 * 10)
        {
            System::getDelayedActions().setActionTime(DelayedActionsBase::GoOutFirst,
                                                      _go_out - Time(0,10));
        }
    }
    else
...
{
    System::getDelayedActions().cancelAction(DelayedActionsBase::GoOutFirst);
    System::getDelayedActions().cancelAction(DelayedActionsBase::GoOutSecond);
    System::getDelayedActions().cancelAction(DelayedActionsBase::GoOutThird);
}
