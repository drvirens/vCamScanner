...
namespace cocos2d
{
	...
	void CCScrollLayer::moveToPageEnded(CCNode* aThis)
	{
		if (m_pDelegate)
	...
	{
    CCPoint thePoint = ccp(pageNumber * -1.f * (m_obContentSize.width - m_fPagesWidthOffset), 0.0f);
    //CCLOG("pageNumber = %d", pageNumber);
		return thePoint;
	}
...
//    CCRepeatForever* repeatAction = CCRepeatForever::create(pulseSequence);
    ...
    CCSequence::createWithTwoActions(
                             ...
                             CCCallFuncN::create(this, callfuncN_selector(CCScrollLayer::moveToPageEnded))
                             );
...
//does not work smoothly but does not crash
//int scrollWidth  = (int)CCDirector::sharedDirector()->getWinSize().width;
//
//    CCEaseBounce* changePage = CCEaseBounce::create(
//                                        CCMoveTo::create(
//                                                  0.3f,
//                                                  ccp( -((pageNumber-1) * scrollWidth ),
//                                                  0)
//                                                  )
//                                                  );
