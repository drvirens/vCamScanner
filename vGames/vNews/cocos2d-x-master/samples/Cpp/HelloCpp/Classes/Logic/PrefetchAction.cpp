...
{
  CCLog("PrefetchAction::do_performPrefetch");
...
//    AppDelegate::configureContentProviders();
    ...
    CCLog("PrefetchAction::do_performPrefetch....Load weather");
    //Load weather
    _waiting_delegates++;
    WeatherHolder::getInstance().getForecast(
                WeatherHolder::DelegatePtr(new WeatherDelegate(this)));

    CCLog("PrefetchAction::do_performPrefetch....Jokes prefetch");
    //Jokes prefetch
    _waiting_delegates++;
...
{
    CCLog("PrefetchAction::delegateFinish");
