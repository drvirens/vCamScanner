...
TimeGuess::TimeGuess()
    : _current_day(0)
{
    std::string wpath = CCFileUtils::sharedFileUtils()->getWritablePath();
...
//viren+
//commented below line because of error on ios : error: use of overloaded operator '<<' is ambiguous (with operand types 'OutputBinaryStream' and 'size_type' (aka 'unsigned long'))
  //os << _map.size();
  size_t theSize = _map.size();
    os << (uint32_t)theSize;
    //viren-
