...
//viren+
  void Forecast::doCopy(const Forecast& aRhs)
  {
      this->_min = aRhs._min;
      this->_max = aRhs._max;
      this->_state = aRhs._state;
      this->_rain_probability = aRhs._rain_probability;
      this->_is_pobability_defined = aRhs._is_pobability_defined;
      this->_last_update = aRhs._last_update;
      //this->_forecast_date = aRhs._forecast_date;
  }
  Forecast::Forecast(const Forecast& aRhs) : _city(aRhs._city)
  {
    doCopy(aRhs);
  }
  Forecast& Forecast::operator=(const Forecast& aRhs)
  {
    if( this != &aRhs)
    {
      doCopy(aRhs);
