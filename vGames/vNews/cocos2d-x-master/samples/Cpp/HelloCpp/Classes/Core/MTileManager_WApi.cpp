...
void MTileManager::WApi::hideWidget(Widget* widget)
{
  ...
  //viren+
void MTileManager::WApi::addChild(cocos2d::CCLayer* child)
{
...
//#if 0 //original code but it crashes sometimes
  CCLOG("MTileManager::WApi::runScript : will call replaceScene");
  if (script.get() != 0) {
    CCLOG("script.get returned valid pointer");
    CCDirector::sharedDirector()->replaceScene(ScriptScene::scene(script));
  } else {
    CCLOG("script.get had NULL pointer so a bomb");
  }
//#endif
}
